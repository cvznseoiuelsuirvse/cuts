...
C_EVENT_CALLBACK client_epoll_callback(struct c_event_loop *loop, int fd, void *data) {
  struct c_wl_connection *conn = data;
  struct c_wl_display *display = c_wl_connection_get_display(conn);
  int ret;

  ret = c_wl_connection_dispatch(conn);
  if (ret) goto out;

  ret = c_wl_connection_flush(conn);
  if (ret == -1) {
